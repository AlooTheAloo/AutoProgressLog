    ...
    .post(
        ...
        async ({ user, set }) => {
            ...
            if (ankiToken || url) {
                ...
                console.log("No Anki config found, skipping NormalSyncer for user:", userId);
            }
            ...
            console.log("Total immersion time for user:", userId, totalImmersion._sum.seconds);
            ...
            const retentionDelta =
                retention - (latestReport?.ankiData?.retention ?? 0);
            const reviews = createdSync.ankiData?.totalCardsStudied ?? 0;
            const reviewsDelta =
                reviews - (latestReport?.ankiData?.totalCardsStudied ?? 0);
            console.log(
                "Anki stats:",
                JSON.stringify({ retention, retentionDelta, reviews, reviewsDelta }, null, 2)
            );
            ...
            if (autoGenTime) {
                ...
                console.log("Next report scheduled at:", new Date(nextReport).toISOString());
            } else {
