...
export async function runChecks(): Promise<boolean> {
  if (await checkInternet()) {
    ...
    if (
      hasSyncEnabled(getConfig().anki.ankiIntegration.profile) &&
    ...
    ankiDTO: getConfig().anki.enabled
      ? {
          ...
          retentionRateDelta: roundTo(
            roundTo(lastEntry.anki.retention, 2) -
              roundTo(lastReport.retention, 2),
            2
          ),

          totalReviews: lastEntry.anki.totalCardsStudied,
          reviewsDelta:
            lastEntry.anki.totalCardsStudied - lastReport.totalCardsStudied,
        }
      : undefined,
    immersionDTO: {
