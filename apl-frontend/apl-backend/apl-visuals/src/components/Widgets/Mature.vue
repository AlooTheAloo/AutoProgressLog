...
const value: ComputedRef<TPlusDelta<number>> = computed(() => {
  ...
  return {
    current: cur,
    delta:
      sortedDataByMature.value.length < 2
        ? cur
        : cur - sortedDataByReport.value[1].matureCardCount,
  };
});
      ...
      <div class="pl-1 flex flex-col w-[91.7%] pt-5">
        <div class="flex flex-grow">
          <div class="flex flex-col">
            <div class="flex flex-col">
              <div class="font-normal tracking-wider">Total mature cards</div>
            ...
            <div class="flex flex-col my-3 flex-grow w-[111.5%]">
              <div class="divide-y divide-[#283B2C] flex-grow">
                <div
                  v-for="(item, index) in lineData?.lines"
                  :key="index"
                  class="h-[35px] flex flex-col"
                >
                  <div
                    class="justify-center gap-2 flex flex-grow items-center font-bold text-sm"
                  >
                    <div>
                      {{ item.reportNo }}
                    </div>
                    <div
                      v-for="index in bgGradient.length"
                      :key="index"
                      class="rounded-full w-[45px] h-[10px]"
                      :style="{
                        backgroundColor:
                        ...
                        boxShadow:
                          index <= item.lineNumber
                            ? '0px 0px 20px 3px ' + bgGradient[index - 1] + '6B'
                            : 'none',
                      }"
                    ></div>
                  </div>
              ...
              <div
                class="text-[#68565F] text-xs mt-5 text-center"
                v-if="(lineData?.lines.length ?? 0) > 1"
              >
                Every bar represents
                {{ pluralize("mature card", lineData?.lineValue, true) }}
                <br />
                Chart represents {{ lineData?.graphMinVal }} →
                {{ pluralize("card", lineData?.graphMaxVal, true) }}
              </div>
            </div>
          </div>
