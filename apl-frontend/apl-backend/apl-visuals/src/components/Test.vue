...
<template>
    <div class="flex flex-col h-full w-full flex-grow gap-12 ">
        <div class="w-full flex gap-8 ">
          <div class="w-0 h-full flex flex-col flex-grow gap-5" v-for="(item, index) in reportLayout" :key="index">
            <component v-for="comp in item" :is="getItemById(comp)" :reportData="props.reportData"/>
          </div>
        </div>
        <div class="w-full">
