...
export async function getProfileDecks(): Promise<
  { name: string; deckCount: number }[] | false
> {
...
export async function getAnkiProfiles(): Promise<{ name: string }[] | null> {
  ...
  const profiles: { name: string }[] | null = await new Promise((res, rej) => {
    prefsDB.all(
      "SELECT name FROM profiles WHERE name NOT IN ('_global')",
      (err, rows: any[]) => {
        res(rows);
      }
    );
...
export async function getAnkiDBPaths(
...
export async function minimizeAnki(cmd: string) {
  ...
  const resp = await new Promise<void | null>(async (res, rej) => {
    var intervalOpen = setInterval(async () => {
      ...
      const targetProcesses = (await getAnkiProcesses()).filter(
        (x) => x.cmd == cmd
      );
      iterations++;
      ...
      if (windows.length == 1) {
        ...
        exec(
          `osascript -e 'tell application "System Events" to set visible of first application process whose unix id is ${pid} to false'`,
          (err, stdout, stderr) => {
            if (err) {
