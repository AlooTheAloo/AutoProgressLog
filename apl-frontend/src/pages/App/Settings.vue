...
function save(){
    window.ipcRenderer.invoke("SetConfig", JSON.stringify(config.value)).then((data:Options) => {
        config.value = data;
        originalConfig.value = data;
        toast.add({ severity: 'success', summary: 'Changes saved!', detail: 'The changes were saved successfully.', life: 5000 });
    })
}
function reset(){
...
function ankiTest(worked:boolean){
    if(worked){
        toast.add({ severity: 'success', summary: 'Connection worked!', detail: 'APL and Anki can communicate properly.', life: 5000 });
    }
    else {
        toast.add({ severity: 'error', summary: 'Connection did not succeed', detail: 'Check your connection settings again. If the issue persists, please open an issue on the github repository.', life: 5000 });
    }
}
...
<template>  
    ...
    <div class="flex flex-col w-full">
        <div class="mb-10" v-html="warningData?.content">
    ...
    <SideBarContainer :currentRoute="router.currentRoute.value.path as appPath">
        <div class="flex flex-col w-full h-full">
            <div class="flex flex-col flex-grow w-full h-full">
                    ...
                    <div class="flex w-full px-2 h-full rounded-lg">
                        ...
                        <div class="w-0">
                            ...
                            <TabPanels unstyled class="py-2">
                                ...
                                <TabPanel value="3">
                                    <AnkiSettings :config="config" @update:config="setConfig" @danger="createWarning" v-on:ankitest:worked="ankiTest"/>
                                </TabPanel>
                                <TabPanel value="4">
                ...
                <div 
                :style="{
                    marginBottom: JSON.stringify(originalConfig) == JSON.stringify(config) ? '-3rem' : '',
                }"

                class="m-5 flex justify-end transition-all duration-300 ease-in-out gap-5">
                    <Button 
                    :aria-hidden="JSON.stringify(originalConfig) == JSON.stringify(config)"
                    :tabindex="JSON.stringify(originalConfig) != JSON.stringify(config) ? '0' : '-1'"
                    @click="save" class="w-72">Save</Button>
                    <Button @click="reset" class="w-72" severity="secondary"
                    :aria-hidden="JSON.stringify(originalConfig) == JSON.stringify(config)"
                    :tabindex="JSON.stringify(originalConfig) != JSON.stringify(config) ? '0' : '-1'"
