...
async function generateReport() {
  try {
    syncing.value = true;
    const maybe:Maybe<DashboardDTO> = await window.ipcRenderer.invoke("GenerateReport");
    if(!("error" in maybe))
    {
      dto.value = maybe;  
    }
...
async function sync(alternate: boolean) {
  try {
    syncing.value = true;
    const maybe:Maybe<DashboardDTO> = await window.ipcRenderer.invoke("Sync", alternate);
    if(!("error" in maybe))
    {
      dto.value = maybe;
          ...
          <div class="flex flex-col flex-grow w-full h-full">
              <div class="flex w-full h-20 items-center px-10 my-5 justify-between">
                <div class="flex flex-col">
                    <h1 class="flex items-center gap-2 bg-gradient-to-r bg-clip-text text-xl xl:text-4xl font-extrabold text-transparent from-[#89BDFF] to-[#40ffff]">
                        Hi, 
                    ...
                    <div class="flex items-center">
                        ...
                        <Button class="h-6 " plain text @click="() => sync(shift)" :loading="syncing">
