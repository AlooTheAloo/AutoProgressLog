...
<template>
  ...
  <div class="p-12 flex flex-col w-2/3 bg-black h-screen">
    ...
    <div
      v-else
      class="flex flex-col flex-grow py-5 justify-start gap-2 text-left"
    >
          ...
          <div class="flex gap-5 items-center justify-between h-full">
            <div class="flex gap-5 min-w-0">
              <img
                ...
                v-bind:src="togglAccount?.pfp_url"
                class="h-8 w-8 rounded-md min-w-8"
              />
              <div
                class="font-semibold flex items-center text-md truncate min-w-0"
              >
                {{ togglAccount?.name }}
              </div>
            </div>
