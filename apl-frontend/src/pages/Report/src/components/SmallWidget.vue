...
const hrValue = computed(() => {
  return props.condense
    ? abbreviateNumber(props.value.current, 2)
    : props.value.current;
});
...
<template>
  <div
    class="w-72 h-36 bg-black rounded-xl flex flex-grow items-center justify-center"
  >
      ...
      <div class="flex-grow flex flex-col justify-center">
        ...
        <div class="font-extrabold text-3xl">
          <div class="flex flex-row">
            <div>{{ hrValue }} {{ props.units ?? "" }}</div>
            <div class="flex-grow" v-if="!props.hideDelta">
              <Delta
                v-bind:override="props.deltaOverride"
                :delta="props.value.delta"
              ></Delta>
            </div>
            <div class="flex-grow" v-if="props.newBest">
              <div
                class="ml-[5px] pb-[2px] h-full flex items-end text-sm text-[#5FFFB2]"
              >
                New Best!
              </div>
            </div>
